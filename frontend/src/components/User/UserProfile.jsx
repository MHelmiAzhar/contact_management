...
export default function UserProfile() {
  ...
  async function fetchCurrentUser() {
    try {
      if (!token) {
        await alertError('User not Logged In')
      }
  ...
  async function handleUpdateProfile(e) {
    ...
    const response = await userUpdateProfile({ name, token })
    const responseBody = await response.json()
    if (response.status === 200) {
      await alertSuccess('Profile updated successfully')
    } else {
      await alertError(responseBody.errors || 'Failed to update profile')
    }
  }
  ...
  async function handleUpdatePassword(e) {
    ...
    if (password !== confirmPassword) {
      return await alertError('Passwords do not match')
    }
    ...
    if (response.status === 200) {
      ...
      await alertError(responseBody.errors || 'Failed to update password')
    }
  }
