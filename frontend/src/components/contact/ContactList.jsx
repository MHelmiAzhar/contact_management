...
import { userDeleteAccount } from '../../lib/api/userApi'
...
export default function ContactList() {
  ...
  async function fetchContacts() {
    const response = await getContacts(token, { page, name, email, phone })
    const responseBody = await response.json()
    if (response.status === 200) {
      setContacts(responseBody.data)
      setTotalPages(responseBody.paging.total_page)
      console.log('Contacts fetched successfully:', responseBody.data)
    } else {
  ...
  async function handleDeleteContact(id) {
    ...
    const response = await userDeleteAccount(token, id)
    const responseBody = await response.json()
    if (response.status === 200) {
      setContacts((prev) => prev.filter((contact) => contact.id !== id))
      console.log('Contact deleted successfully:', responseBody.data)
  ...
  useEffect(() => {
    // Reset page to 1 when search criteria changes
    setPage(1)
  }, [name, email, phone])
          ...
          >
            <i
              className={`fas text-lg ${
                showSearchForm ? 'fa-chevron-up' : 'fa-chevron-down'
              }`}
              id="toggleSearchIcon"
            ></i>
          </button>
        ...
        <nav className="flex items-center space-x-3 bg-gray-800 bg-opacity-80 rounded-xl shadow-custom border border-gray-700 p-3 animate-fade-in">
          ...
          {page < totalPages && (
            <a
              ...
              className="px-4 py-2 bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 flex items-center"
            >
              <i className="fas fa-chevron-right mr-2"></i> Next
            </a>
          )}
        </nav>
      </div>
